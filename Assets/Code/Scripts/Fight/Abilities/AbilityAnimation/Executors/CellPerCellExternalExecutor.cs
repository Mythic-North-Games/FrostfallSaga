...
namespace FrostfallSaga.Fight.Abilities.AbilityAnimation
{
    ...
    public class CellPerCellExternalExecutor : AExternalAbilityAnimationExecutor
    {
        ...
        public override void Execute(Fighter fighterThatExecutes, FightCell[] abilityCells, GameObject projectilePrefab)
        {
            ...
            _projectilesToFinishedTranslation = new List<GameObject>();

            if (projectilePrefab.GetComponent<FighterCollider>() == null)
                Debug.LogWarning(
                    "No fighter collider found on projectile prefab. onFighterTouched event will not be triggered.");

            foreach (FightCell targetCell in abilityCells)
            {
                GameObject projectile = Object.Instantiate(projectilePrefab, targetCell.transform);
                SetupProjectileColliderEventsIfAny(projectile);
                projectile.transform.position = new Vector3(999999, 999999); // Hide before launching
        ...
        {
            projectileTransform.gameObject.GetComponent<GameObjectTranslator>().onTargetLocationReached -=
                OnProjectileReachedEndLocation;
            ...
            if (_projectilesToFinishedTranslation.All(projectile => !projectile.activeInHierarchy))
            {
                ...
                _projectilesToFinishedTranslation.ForEach(projectile => Object.Destroy(projectile));
            }
        }
    }
}