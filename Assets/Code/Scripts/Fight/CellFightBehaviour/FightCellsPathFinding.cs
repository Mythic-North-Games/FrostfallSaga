...
{
    /// <summary>
    /// Expose methods for pathfinding between cells in a fight grid.
    /// </summary>
    public static class FightCellsPathFinding
    {
        /// <summary>
        /// Returns the shorter path between the given two cells that are inside the given grid.
        /// </summary>
        /// <param name="hexGrid">The fight grid that contains at least the two cells.</param>
        ...
        public static Cell[] GetShorterPath(HexGrid hexGrid, Cell startCell, Cell endCell)
        {
            ...
            while (frontier.Count > 0)
            {
                ...
                foreach (Cell neighbor in FightCellNeighbors.GetNeighbors(hexGrid, currentCell))
                {
